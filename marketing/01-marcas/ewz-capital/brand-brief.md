# EWZ Capital — brand brief

> Documento-raiz da marca. Campanha, pauta ou peça que contrarie este documento
> não vai ao ar. Revisão trimestral.
>
> ⚠️ A CONFIRMAR — hipóteses do diretor de marketing, pendentes de validação com sócios.

## O que é

| | |
|---|---|
| Categoria | Assessoria de investimentos credenciada ao BTG Pactual (desde 2021; antes XP) |
| Público | Investidor qualificado, family office, institucional |
| Papel no grupo | Ponta institucional — credibilidade e track record |
| Site | `capital.ewz.com.br` — ⚠️ `ewzcapital.com.br` roda em paralelo, definir o canônico |

> Dados de mercado levantados em set/2026: [`mapeamento digital`](../../05-inteligencia/2026-09-mapeamento-digital.md).

## Posicionamento

_(Uma frase: para quem, o que entregamos, por que somos diferentes.)_

> Para **[quem]**, a EWZ Capital é **[categoria]** que **[benefício]**,
> diferente de **[alternativa]** porque **[razão para crer]**.

## Razões para crer

Em marca institucional, afirmação sem prova é ruído. Listar apenas o que é verificável:

- Track record: 
- Time e credenciais: 
- Tese de investimento: 
- Governança e auditoria: 

## Tom de voz

| É | Não é |
|---|---|
| Sóbrio | Empolgado |
| Técnico e preciso | Jargão vazio |
| Direto | Promessa de retorno |
| Transparente sobre risco | Otimista por padrão |

Palavras que evitamos: "garantido", "sem risco", "oportunidade imperdível",
"rentabilidade certa". Ver `compliance.md`.

## O que a marca faz e o que não faz

**Faz:** análise, carta ao investidor, conteúdo técnico, presença em LinkedIn e eventos do setor.

**Não faz:** conteúdo de meme, promessa de retorno, trend de rede social, disputa de preço.

## Posição dentro do EWZ Group

A EWZ Capital é uma das três marcas do **EWZ Group**, ao lado de Mundo Invest e Ilha Forte.

| Pergunta | Resposta |
|---|---|
| O grupo aparece nas peças da EWZ Capital? | ⚠️ definir — ver `../README.md` |
| Assinatura em uso | ⚠️ ex.: "EWZ Capital, uma empresa do EWZ Group" |
| Relação com Mundo Invest | ⚠️ definir |
| Relação com Ilha Forte | ⚠️ definir |
| Lead desta marca pode ser abordado pelas outras? | ⚠️ **não**, até haver base legal de LGPD definida |

Enquanto a arquitetura de marcas não for decidida, trate a marca como independente
nas peças públicas.

## Concorrentes de referência

| Concorrente | O que fazem bem | Nossa diferença |
|---|---|---|
| | | |
